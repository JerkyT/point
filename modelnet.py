...
class ModelNet40Ply2048(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train' and self.transformc is not None:
            datac = {'pos': pointcloud,
                 'y': label
                 }

            data['posc'] = datac['pos']

            if 'heights' in datac.keys():
                data['xc'] = torch.cat((datac['pos'], datac['heights']), dim=1)
            else:
                data['xc'] = datac['pos']
        ####
        ...
        if self.shapley and self.partition == 'train':
            data['shap_p'] = self.shapleys[item][:, :, int(label)].permute(1,0).contiguous()
